...
public class java202502211 {
	...
	public static void main(String[] args) {
		...
		//point 가 2인거 찾기 위한 if문 
		//그리고 찾았으면 그자리에서 출력할 것인지 아니면 반환할것인지 
		//내용 동작하기 위해 필요한것
		//-coutse배열이 필요 ->매개변수 
 ...
 *	-이후에는 이전에 만든 메서드를 사용하면 되서 시간이 단축된다
		   	-그래서 메서드도 설계를 따로 해주기는한다 
 * 3.메서드에 필요한것을 제공(매개변수)
 *  -여러개의 메서드는 서로 협력하여 일을 진행한다.
 *  -물양동이를 혼자서 200m걸어가 옮기는것 보다 
 *  -여러명이 한줄로서서 전달해서 옮기면 개개인이 덜 힘들다 
 *  -최종 목적지까지 도달하기 위해 다음 사람에게 필요한것( 양동이-매개변수)
 *  를 전달해준다.
 *4. 다음 할일을 하기위해 필요한것 돌려받기 (반환-return)
